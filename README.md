...
| IPC（ウィンドウ間通信） | Unix ドメインソケットによる外部制御。`cm ctl` コマンドでキー送信・画面キャプチャ・ターミナル一覧取得。AI エージェント連携対応 |
| リネーム | ターミナル名を後から変更可能 |
...
| **Infrastructure** | 具象実装（PTY, 画面, TUI, 通知, IPC, DI） | ratatui, crossterm, portable-pty, vte, vt100, notify-rust, serde, serde_json |
...
合計 **1269** ユニットテスト。各モジュールごとの内訳は以下の通りです。

```mermaid
pie title ユニットテスト構成 (1269件)
    "AppRunner (288)" : 288
    "VteScreenAdapter (177)" : 177
    "InputHandler (133)" : 133
    "Vt100ScreenAdapter (124)" : 124
    "TerminalView (79)" : 79
    "MiniTerminalView (43)" : 43
    "TerminalUsecase (65)" : 65
    "TuiController (57)" : 57
    "IpcCommand (39)" : 39
    "QuickSwitcher (40)" : 40
    "IPC (70)" : 70
...
| モジュール | テスト数 | テスト対象 |
|-----------|---------|-----------|
| `AppRunner` | 288 | イベントループ、スクロールバック（メイン/ミニ）、カーソル自由移動（Phase 19）、フォーカス制御、ミニターミナル管理、クイックスイッチャー統合、スクロールバック検索、ヤンクバッファ、Visual 選択モード、IPC コマンドハンドラ |
| `VteScreenAdapter` | 177 | ANSI パース、セルグリッド、カーソル移動、代替画面、スクロールリージョン、ワイド文字、OSC タイトル、通知 |
| `InputHandler` | 133 | ステートマシン、プレフィックスキー、タイムアウト、アプリケーションカーソルキー、ブラケットペースト、スクロールバックモード（h/l/0/$）、検索モード、メモ編集モード、ヘルプ表示、ミニターミナル入力、ヤンク/Visual/ペースト/PasteToTarget キーバインド |
...
| `TerminalUsecase` | 65 | CRUD 操作、ポーリング、通知収集、リネーム、メモ操作、get_terminal_by_id、エラーハンドリング |
...
| `IpcCommand` | 39 | IPC コマンド・レスポンスドメイン型、WindowInfo 構造体 |
| `QuickSwitcher` | 40 | オーバーレイ描画、クエリ入力、選択ハイライト、マッチ文字ハイライト、スクロール、小画面対応 |
| `Sidebar` | 32 | ターミナル一覧描画、動的 CWD 表示、通知マーク、メモインジケータ |
| `Protocol` | 31 | JSON パース・シリアライズ、全コマンド/レスポンス型、エラーケース |
| `CliClient` | 30 | cm ctl サブコマンド、リクエスト構築、-t フラグ解析、レスポンス表示 |
| `HelpOverlay` | 22 | ヘルプオーバーレイ描画、カテゴリ表示、キーバインド一覧（h/l・0/$ を含む）、検索・ヤンク・Visual キーバインド表示、小画面対応 |
...
CLI Manager 内で実行中の AI エージェント（Claude Code など）から、他のターミナルを操作できます。子プロセスは `CLI_MANAGER_SOCK` 環境変数を通じてソケットパスを取得できます。
...
{"command": "send-keys", "args": ["ls", "Enter"]}
{"command": "send-keys", "target": 2, "args": ["cargo test", "Enter"]}
{"command": "capture-pane"}
{"command": "capture-pane", "target": 1}
{"command": "paste-buffer"}
{"command": "set-buffer", "args": ["text to set"]}
{"command": "show-buffer"}
```
