...
合計 **1218** ユニットテスト。各モジュールごとの内訳は以下の通りです。

```mermaid
pie title ユニットテスト構成 (1218件)
    "AppRunner (237)" : 237
    "VteScreenAdapter (177)" : 177
    "InputHandler (126)" : 126
...
| モジュール | テスト数 | テスト対象 |
|-----------|---------|-----------|
| `AppRunner` | 237 | イベントループ、スクロールバック（メイン/ミニ）、フォーカス制御、ミニターミナル管理、クイックスイッチャー統合、スクロールバック検索、ヤンクバッファ、Visual 選択モード、IPC コマンドハンドラ |
| `VteScreenAdapter` | 177 | ANSI パース、セルグリッド、カーソル移動、代替画面、スクロールリージョン、ワイド文字、OSC タイトル、通知 |
| `InputHandler` | 126 | ステートマシン、プレフィックスキー、タイムアウト、アプリケーションカーソルキー、ブラケットペースト、スクロールバックモード、検索モード、メモ編集モード、ヘルプ表示、ミニターミナル入力、ヤンク/Visual/ペースト/PasteToTarget キーバインド |
...
| `TuiController` | 49 | AppAction ディスパッチ、状態管理、リネーム・メモ・ヘルプ・ミニターミナル・クイックスイッチャー・検索・ヤンク/ペースト/Visual/PasteToTarget 操作 |
