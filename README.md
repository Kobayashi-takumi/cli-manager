...
| プレフィックスキー | tmux ライクな `Ctrl+t` プレフィックスモデル |
...
起動すると TUI が表示されます。最初のターミナルを作成するには `Ctrl+t` → `c` を押してください。

## 操作方法

### キーバインド一覧

すべての操作コマンドは **プレフィックスキー `Ctrl+t`** の後に入力します。

| キーバインド | アクション |
|---|---|
| `Ctrl+t` → `c` | 新しいターミナルを作成 |
| `Ctrl+t` → `d` | アクティブターミナルを削除（実行中なら確認あり） |
| `Ctrl+t` → `n` | 次のターミナルを選択 |
| `Ctrl+t` → `p` | 前のターミナルを選択 |
| `Ctrl+t` → `1`〜`9` | 番号指定でターミナルをジャンプ |
| `Ctrl+t` → `Ctrl+t` | 子プロセスに `Ctrl+t` を送信 |
| `Ctrl+t` → `q` | アプリケーション終了 |
| その他のキー | アクティブターミナルの stdin へパススルー |

### プレフィックスキーの仕組み

`Ctrl+t` は tmux の `Ctrl+b` と同様のプレフィックスキーです。InputHandler が以下のステートマシンで管理します。

```mermaid
stateDiagram-v2
    [*] --> Normal
    Normal --> PrefixWait : Ctrl+t 押下
    PrefixWait --> Normal : コマンドキー入力\n(c/d/n/p/1-9/q)
    PrefixWait --> Normal : 1秒タイムアウト\n(Ctrl+t を子プロセスへ送信)
    PrefixWait --> Normal : Ctrl+t 再押下\n(Ctrl+t を子プロセスへ送信)
```

**ポイント:**
- `Ctrl+t` を押すと 1 秒間コマンド入力を待機します
- 1 秒以内にコマンドキーを押さなかった場合、`Ctrl+t` が子プロセスにそのまま送られます
- 子プロセスに `Ctrl+t` 自体を送りたい場合は `Ctrl+t` → `Ctrl+t` と 2 回押します
...
stateDiagram-v2
    ...
    Exited --> Removed : Ctrl+t → d
    Running --> Removed : Ctrl+t → d\n(確認ダイアログ後)
    Removed --> [*]
