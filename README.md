...
| IPC（ウィンドウ間通信） | Unix ドメインソケットによる外部制御。`cm ctl` コマンドでキー送信・画面キャプチャ・ターミナル管理。AI エージェント連携対応 |
| MCP Server | MCP（Model Context Protocol）対応。`cm mcp-server` で stdio サーバーを起動し、Claude Code 等の AI エージェントからターミナルを直接操作可能 |
...
│   │   ├── tool_definitions.rs          # 10 ツールのスキーマ定義
...
合計 **1424** ユニットテスト。各モジュールごとの内訳は以下の通りです。

```mermaid
pie title ユニットテスト構成 (1424件)
    "AppRunner (296)" : 296
...
| モジュール | テスト数 | テスト対象 |
|-----------|---------|-----------|
| `AppRunner` | 296 | イベントループ、スクロールバック（メイン/ミニ）、カーソル自由移動（Phase 19）、フォーカス制御、ミニターミナル管理、クイックスイッチャー統合、スクロールバック検索、ヤンクバッファ、Visual 選択モード、IPC コマンドハンドラ（ウィンドウ管理含む） |
...
| `TerminalUsecase` | 75 | CRUD 操作、ポーリング、通知収集、リネーム、メモ操作、get_terminal_by_id、close_by_id、select_by_id、rename_by_id、エラーハンドリング |
...
| `IpcCommand` | 62 | IPC コマンド・レスポンスドメイン型、WindowInfo 構造体、ウィンドウ管理コマンド |
| `QuickSwitcher` | 40 | オーバーレイ描画、クエリ入力、選択ハイライト、マッチ文字ハイライト、スクロール、小画面対応 |
| `Sidebar` | 32 | ターミナル一覧描画、動的 CWD 表示、通知マーク、メモインジケータ |
| `Protocol` | 41 | JSON パース・シリアライズ、全コマンド/レスポンス型（ウィンドウ管理含む）、エラーケース |
| `CliClient` | 51 | cm ctl サブコマンド（ウィンドウ管理含む）、リクエスト構築、-t フラグ解析、レスポンス表示 |
| `MCP Server` | 72 | MCP JSON-RPC ハンドリング、10 ツール定義、ツール→IPC コマンド変換、レスポンス変換、パラメータバリデーション |
| `SocketDiscovery` | 10 | ソケットパスの書き出し・読み取り・クリーンアップ、パーミッション確認 |
