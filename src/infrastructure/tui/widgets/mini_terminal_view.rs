...
pub fn render(
    ...
    let block = if let Some((offset, max)) = scrollback_info {
...
mod tests {
    ...
    fn render_shows_border_and_title() {
        ...
        terminal
            .draw(|frame| {
                ...
                render(frame, area, None, None, false, true, None);
            })
            .unwrap();
    ...
    fn render_focused_has_yellow_border() {
        ...
        terminal
            .draw(|frame| {
                let area = Rect::new(0, 0, 40, 10);
                render(frame, area, None, None, false, true, None); // is_focused = true
            })
            .unwrap();
        let buf = terminal.backend().buffer();
    ...
    fn render_unfocused_has_dark_gray_border() {
        ...
        terminal
            .draw(|frame| {
                let area = Rect::new(0, 0, 40, 10);
                render(frame, area, None, None, false, false, None); // is_focused = false
            })
            .unwrap();
        let buf = terminal.backend().buffer();
    ...
    fn render_cells_appear_inside_border() {
        ...
        terminal
            .draw(|frame| {
                ...
                render(frame, area, Some(&cells), None, false, true, None);
            })
            .unwrap();
        let buf = terminal.backend().buffer();
    ...
    fn render_cursor_visible_shows_reverse_video() {
        ...
        terminal
            .draw(|frame| {
                let area = Rect::new(0, 0, 20, 6);
                render(frame, area, Some(&cells), Some(cursor), true, true, None);
            })
            .unwrap();
        let buf = terminal.backend().buffer();
    ...
    fn render_cursor_invisible_does_not_show_reverse() {
        ...
        terminal
            .draw(|frame| {
                let area = Rect::new(0, 0, 20, 6);
                render(frame, area, Some(&cells), Some(cursor), false, true, None);
            })
            .unwrap();
        let buf = terminal.backend().buffer();
    ...
    fn render_scrollback_indicator_style_is_yellow_bold() {
        ...
        terminal
            .draw(|frame| {
                let area = Rect::new(0, 0, 40, 10);
                render(frame, area, None, None, false, true, Some((10, 50)));
            })
            .unwrap();
        let buf = terminal.backend().buffer();
